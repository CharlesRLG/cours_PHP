...
//1- Vérification de l'existance du cookie historique
if (isset($_COOKIE["historique"])) {
    ...
    $message = "<p>cookie inexistant</p>";
    // création du cookie historique
    $fichier = bin2hex(random_bytes(10));
    setcookie("historique",$fichier,time()+60*60*24); //création cookie
    file_put_contents($fichier.".log",date("d/m/Y H:i:s")); // création du journal.log
    echo "Félicitation ! Première connection !"; // affichage première connection
    header('Refresh: 5; URL=exercice_cookie_connection.php');
}
